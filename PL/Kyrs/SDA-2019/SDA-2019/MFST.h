...
namespace MFST
{
	...
	struct Mfst						// магазинный автомат
	{
		...
		bool savestate(Out::OUT out);		// сохранить состояние автомата
		bool reststate(Out::OUT out);		// восстановить состояние автомата
		bool push_chain(						// поместить цепочку правила в стек
			GRB::Rule::Chain chain		// цепочка правил
		);
		Mfst::RC_STEP step(Out::OUT out);	// выполнить шаг автомата
		bool start(Out::OUT out);	// запустить автомат
		bool savediagnosis(
			RC_STEP pprc_step	// код завершения шага
		);
		void printrules(Out::OUT out);// вывести последовательность правил (дерево разбора)
