...
#define TS(n) Rule::Chain::T(n)
	Greibach greibach(NS('S'), TS('$'),       // стартовый символ, дно стека
		...
		Rule(NS('S'), GRB_ERROR_SERIES + 0,   // Неверная структура программы
			3,
			Rule::Chain(8, TS('m'), TS('{'), NS('N'), TS('e'), NS('E'), TS(';'), TS('}'), TS(';')),
			Rule::Chain(14, TS('f'), TS('t'), TS('i'), TS('('), NS('F'), TS(')'), TS('{'), NS('N'), TS('e'), NS('E'), TS(';'), TS('}'), TS(';'), NS('S')),
			Rule::Chain(13, TS('f'), TS('t'), TS('i'), TS('('), NS('F'), TS(')'), TS('{'), TS('e'), NS('E'), TS(';'), TS('}'), TS(';'), NS('S'))
		),
		Rule(NS('N'), GRB_ERROR_SERIES + 1,   // Ошибка в инструкциях или конструкциях
			8,  
			...
			Rule::Chain(7, TS('c'), TS('('), NS('W'), TS(')'), TS('['), NS('N'), TS(']')),
		...
		Rule(NS('M'), GRB_ERROR_SERIES + 3,   //  Ошибочный оператор
			2, 
	...
	char* Rule::getCRule(char* b, short nchain) // получить правило в виде И->цепочка
	{
		char bchain[200];
