...
#define MFST_TRACE7		std::cout<<std::setw(4)<<std::left<<state.lenta_position<< ": " << std::setw(20)<<std::left<<rule.getCRule(rbuf,state.nrulechain)<<std::endl;
	...
	bool Mfst::start() {
		...
		case LENTA_END:  MFST_TRACE4("------>LENTA_END")
			...
			std::cout << std::setw(4) << std::left << 0 << ": всего строк " << lenta_size << ", синтаксичекий анализ выполнен без ошибок" << std::endl;
			rc = true;
			break;
